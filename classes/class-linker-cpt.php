...
class Linker_CPT {
	...
	public function register_post_type() {
		...
		$args = array(
			...
			'rewrite'             => array( 'slug' => apply_filters( 'linker_prefix_slug', $this->redirect ),'with_front' => false,),
		);
		...
		switch ( $column ) {
			...
			case 'thumbnail' :
                if ( has_post_thumbnail() ){
                    the_post_thumbnail('thumbnail', ['class' => 'linker-thumbnail']);
                }
                else {
                    echo __('Thumbnail is not set','linker');
                }
				break;

            case 'order' :
                echo get_post_field('menu_order', $post->ID, true) ;
                break;
		}
	}
	...
	public function render_meta_box( $post ) {
        ...
        $field_id = '_linker_target';

        echo strtr( '<p><label for="{name}"><input type="checkbox" id="{name}" name="{name}" value="{value}" {checked}>{label}</input></label></p>', array(
            '{label}' => __( 'new window', 'linker' ),
            '{name}' => $field_id,
            '{value}' => esc_attr( get_post_meta( $post->ID, $field_id, true ) ),
            '{checked}' => checked( get_post_meta( $post->ID, $field_id, true ), 1, false ),
        ));
	...
	public function save_post( $post_id ) {
        ...
        if ( isset( $_POST['_linker_target'] ) ) {
            update_post_meta( $post_id, '_linker_target', true );
        }
        else {
            delete_post_meta( $post_id, '_linker_target' );
        }
		...
		if ( isset( $_POST['_linker_redirect'] ) ) {
            ...
            // Adding post meta with 0 on create post
	...
	public function linker_dashboard_widget_function() {
		...
		?>
		<table class="wp-list-table widefat fixed striped table-view-list linker">
			<thead>
	...
	public function clicks_orderby( $query ) {
		...
		if ( 'linker_clicks' == $orderby ) {
            ...
            // FIXED: return value click 0 when order by !!
		}
